...
export const ShopSettingsPage = () => {
  ...
  const organization = orgData?.data as Organization | undefined;
  ...
  const handleBrandingUpdate = async (values: Record<string, unknown>) => {
    try {
      const primaryColor = (values.primaryColor as { toHexString?: () => string })?.toHexString?.() || (values.primaryColor as string);
      const secondaryColor = (values.secondaryColor as { toHexString?: () => string })?.toHexString?.() || (values.secondaryColor as string);
