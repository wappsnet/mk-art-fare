...
export const ShopAnalyticsPage = () => {
  ...
  const stats = statsData?.data as AnalyticsData | undefined;
                ...
                <FullWidthSpace direction="vertical">
                  {stats.byStatus.map((item) => (
                    <FlexBetween key={item.status}>
                      ...
                      <Text>
                        {item.count} orders ($
                        {(typeof item.revenue === 'number' ? item.revenue : Number.parseFloat(item.revenue as string)).toFixed(2)})
                      </Text>
                    </FlexBetween>
                  ))}
                </FullWidthSpace>
                  ...
                  {stats.topProducts.map((product) => (
                    <FlexBetween key={product.name}>
                      ...
                      <Text>
                        ...
                        {(typeof product.revenue === 'number' ? product.revenue : Number.parseFloat(product.revenue as string)).toFixed(2)})
                      </Text>
                    </FlexBetween>
                  ))}
                </FullWidthSpace>
                  ...
                  {stats.topEvents.map((event) => (
                    <FlexBetween key={event.name}>
                      ...
                      <Text>
                        ...
                        {(typeof event.revenue === 'number' ? event.revenue : Number.parseFloat(event.revenue as string)).toFixed(2)})
                      </Text>
                    </FlexBetween>
                  ))}
                </FullWidthSpace>
