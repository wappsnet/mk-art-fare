 ...
 * @param {boolean} hasCustomFields - Whether to include custom fields join
 * @returns {string} Base query string
 */
function buildBaseQuery(selectClause, hasCustomFields) {
  let query = `
    ${selectClause}
    FROM products p
...
/**
 * Add filter conditions and params
 * @param {Object} filters - Filter options
 * @param {Array} conditions - Conditions array to populate
 * @param {Array} params - Params array to populate
 */
function addFilterConditions(filters, conditions, params) {
  ...
  addCustomFieldConditions(filters, conditions, params);
 ...
 * @param {Array} params - Params array to populate
 */
function addCustomFieldConditions(filters, conditions, params) {
...
export async function buildProductQuery(filters = {}) {
  const conditions = [];
  const params = [];
  const baseQuery = buildBaseQuery('SELECT DISTINCT p.*', !!filters.customFields);

  addFilterConditions(filters, conditions, params);
...
export async function buildProductCountQuery(filters = {}) {
  const conditions = [];
  const params = [];
  const countQuery = buildBaseQuery('SELECT COUNT(DISTINCT p.id) as total', !!filters.customFields);
