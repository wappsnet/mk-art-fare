...
router.get('/', optionalAuth, asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (req.user) {
    ...
    if (!cartId) {
      const result: any = await query('INSERT INTO carts (user_id) VALUES (?)', [req.user.userId]);
      cartId = result.insertId;
    }
  } else {
    const sessionId = req.query.sessionId as string;
    if (sessionId) {
      const cart: any = await query('SELECT * FROM carts WHERE session_id = ?', [sessionId]);
      cartId = cart.length > 0 ? cart[0].id : null;
    }
  }
...
// Add to cart
router.post('/items', optionalAuth, asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (req.user) {
    ...
    if (cart.length > 0) {
      ...
      const result: any = await query('INSERT INTO carts (session_id) VALUES (?)', [sessionId]);
      cartId = result.insertId;
    }
  } else {
  ...
  const existing: any = await query('SELECT * FROM cart_items WHERE cart_id = ? AND product_id = ?', [cartId, productId]);

  if (existing.length > 0) {
    await query('UPDATE cart_items SET quantity = quantity + ? WHERE cart_id = ? AND product_id = ?', [quantity, cartId, productId]);
  } else {
...
// Update cart item
router.patch('/items/:productId', optionalAuth, asyncHandler(async (req: AuthRequest, res) => {
...
// Clear cart
router.delete('/', optionalAuth, asyncHandler(async (req: AuthRequest, res) => {
  let cartId: number | null = null;
