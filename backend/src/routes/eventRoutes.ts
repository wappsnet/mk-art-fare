...
import { AuthRequest, UserRole, EventBookingStatus } from '../types';
import { AppError } from '../middleware/errorHandler';
import { generateSlug, generateBookingNumber, getPaginationParams } from '../utils/helpers';
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  let queryStr = 'SELECT e.*, o.name as organization_name FROM events e LEFT JOIN organizations o ON e.organization_id = o.id WHERE e.is_active = TRUE';
  const params: any[] = [];
  ...
  const events: any = await query(queryStr, params);
...
router.get('/:slug', asyncHandler(async (req, res) => {
  ...
  const tickets: any = await query('SELECT * FROM event_tickets WHERE event_id = ?', [event[0].id]);
...
router.post('/', authenticate, authorize(UserRole.ARTIST, UserRole.ADMIN), asyncHandler(async (req: AuthRequest, res) => {
  const { organizationId, title, description, eventType, venue, address, startDate, endDate, featuredImageUrl, tickets } = req.body;
  ...
  try {
    ...
    const eventResult: any = await connection.execute(
      `INSERT INTO events (organization_id, title, slug, description, event_type, venue_name,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        organizationId || null, title, slug, description || null, eventType || null, venue?.name || null,
        address?.line1 || null, address?.line2 || null, address?.city || null, address?.state || null,
        address?.country || null, address?.postalCode || null, address?.latitude || null, address?.longitude || null,
        startDate, endDate, featuredImageUrl || null
      ]
    );
    ...
    const event: any = await query('SELECT * FROM events WHERE id = ?', [eventId]);
...
// Book event ticket
router.post('/:eventId/book', authenticate, asyncHandler(async (req: AuthRequest, res) => {
  const eventId = parseInt(req.params.eventId);
  const { ticketId, quantity, attendeeInfo } = req.body;

  const ticket: any = await query('SELECT * FROM event_tickets WHERE id = ? AND event_id = ?', [ticketId, eventId]);
  ...
  try {
    ...
    const bookingResult: any = await connection.execute(
      `INSERT INTO event_bookings (booking_number, event_id, ticket_id, user_id, quantity, total_price, status,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        bookingNumber, eventId, ticketId, req.user!.userId, quantity, totalPrice, EventBookingStatus.CONFIRMED,
        attendeeInfo.name, attendeeInfo.email, attendeeInfo.phone || null
      ]
    );
    ...
    const booking: any = await query('SELECT * FROM event_bookings WHERE id = ?', [bookingResult[0].insertId]);
...
// Get user bookings
router.get('/bookings/my', authenticate, asyncHandler(async (req: AuthRequest, res) => {
